...
DueTimer timer2ms = DueTimer(0);
DueTimer timer15us = DueTimer(1);
DueTimer timer10us = DueTimer(2);
...
void setup() {
  // Initialisierung der Timer
	timer2ms.attachInterrupt(timer2msHandler).setPeriod(2).stop();
	timer15us.attachInterrupt(timer15usHandler).setFrequency(66667).stop();
	timer10us.attachInterrupt(timer10usHandler).setFrequency(100000).stop();
